...
class InferenceEngine:
    ...
    def _init_model(self):
        ...
        for _ in range(self.num_sessions):
            ...
            # Get the correct vocabulary size from the logits tensor in session bindings
            vocab_size = None
            for binding in session.bindings:
                if binding.name == "logits":
                    if len(binding.dims) >= 3:
                        # 3D tensor: [batch, seq, vocab]
                        vocab_size = binding.dims[2]
                        break
                    elif len(binding.dims) == 1:
                        # 1D tensor: extract vocab_size from the expected dimensions
                        total_size = binding.dims[0]
                        # Common vocab sizes for this model type
                        expected_vocab_sizes = [151936, 151665, 152064]
                        for expected_vocab in expected_vocab_sizes:
                            if total_size % expected_vocab == 0:
                                vocab_size = expected_vocab
                                break
                        if vocab_size is None:
                            # Fallback: use the largest reasonable vocab size
                            vocab_size = max([v for v in expected_vocab_sizes if v <= total_size])
                        break

            if vocab_size is None:
                # Fallback to tokenizer vocabulary size if logits tensor not found
                vocab_size = self.vocab_size

            # Pre-allocate buffers per session
            prefill_buffer = np.zeros(
                (self.prefill_bsz, 1, vocab_size), dtype=np.float32
            )
            decode_buffer = np.zeros(
                (self.full_batch_size, self.num_logits_to_keep, vocab_size), dtype=np.float32
            )
